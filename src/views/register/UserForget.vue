<template>
  <div class="wrapper">
    <div class="register-container">
      <h2 class="register-header">忘记密码</h2>
      <el-form :model="userForget" ref="registerForm" :rules="rules" class="register-form">

        <el-form-item prop="mobile">
          <el-input size="medium" style="margin: 5px 0" prefix-icon="el-icon-mobile"
                    v-model="userForget.mobile" placeholder="请输入手机号"></el-input>
        </el-form-item>

        <el-form-item prop="verificationCode">
          <el-input size="medium" style="margin: 1px 0" prefix-icon="el-icon-message"
                    v-model="userForget.verificationCode" placeholder="请输入验证码">
        ...
        <el-form-item prop="password">
          <el-input size="medium" style="margin: 5px 0" prefix-icon="el-icon-lock"
                    v-model="userForget.password" placeholder="请输入密码" show-password></el-input>
        </el-form-item>

        <el-form-item prop="confirmPassword">
          <el-input size="medium" style="margin: 5px 0" prefix-icon="el-icon-lock"
                    v-model="userForget.confirmPassword" placeholder="请再次输入密码" show-password></el-input>
        </el-form-item>


        <div style="margin: 5px 0; text-align: right;">
          <el-button type="primary" size="medium" autocomplete="off" round @click="changePassword">确认修改</el-button>
...
export default {
  ...
  methods: {
    sendVerificationCode() {
      ...
      // 构造请求参数，如手机号码
      const params = {
        account: this.userForget.mobile
      };
      let _this = this;
      console.log("获取验证码")
      this.request.post('http://5735bc3c.r7.cpolar.top/user/get_reset_phone_code', params)
          .then(response => {
    ...
    checkConfirmPassword(rule, value, callback) {
      if (value !== this.userForget.password) {
    ...
    changePassword() {
      this.$refs["registerForm"].validate((valid) => {
        if (valid) {
          const params = {
            repassword:this.userForget.confirmPassword,
            account: this.userForget.mobile,
            code: this.userForget.verificationCode,
            password: this.userForget.password,
          };

          this.request.post('http://5735bc3c.r7.cpolar.top/user/resetPassword', params)
              .then(response => {
                // 注册成功处理
                console.log(response)
                if(response.code==='200'){
              ...
              .catch(error => {
                ...
                console.error(error);
                this.$message.error("修改失败"); // 可根据需要显示错误提示
              });
        } else {
          return false;
        }
