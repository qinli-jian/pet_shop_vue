  ...
  <div style="background-color:#FDFDFD">
    ...
    <div class="details">
            ...
            <div class="card-content1">
              <div class="card-details">
                <div class="item22" v-if="productDetails.commodity !== undefined">
                  ...
                  <p style="color: #9BABB8">总评论: {{ total }}</p>
                  <el-rate v-model="averRating" disabled show-score text-color="#ff9900" score-template="{value}">
                  </el-rate>
                </div>
                <hr>
                <div class="item22">
                  ...
                  <div style="display: flex;flex-direction:column">
                    ...
                    <div style="display: flex;flex-direction: row;align-items: flex-start;flex-wrap: wrap;"
                      v-if="productDetails.commodity !== undefined">
                      <el-button style="margin: 10px;"
                        :type="currentSelectSpec === item.specifications_id ? 'primary' : ''"
                        v-for="item in productDetails.commodity.specification_price" :key="item.specifications_id"
                        size="mini" @click="selectOption(item)">
                ...
                <div class="item22"><b style="margin-top: 10px ;color: red;font-size: 20px ">总价:{{ (price *
                  amount).toFixed(2) }}元</b></div>
      ...
      <div style="width: 1000px">
        <el-collapse style="width:100%" v-model="activeNames" @change="handleChange">
          <el-collapse-item style="width:100%;" title="商品描述" name="1">
            <div class="images-area" v-if="productDetails.commodity !== undefined">
              <img style="width:100%;" :src="globalVar.STATIC_NAME + item" :key="index"
                v-for="(item, index) in productDetails.commodity.description_images" />
            </div>
              ...
              <div class="comment-box" v-for="(comment, index) in comments" :key="index">
                <div class="inner-box" v-if="comment.reply_to_comment_id === '-1'">
                  <div class="comment-header">
                    <el-avatar
                      :src="comment.avatar == null ? 'https://cube.elemecdn.com/0/88/03b0d39583f48206768a7534e55bcpng.png' : globalVar.STATIC_NAME + comment.avatar"></el-avatar>
                    ...
                    <div class="time" style="flex-wrap: nowrap" v-if="comment.rating != '-1'">
                      <el-rate v-model="comment.rating" disabled show-score text-color="#ff9900" score-template="{value}">
                      </el-rate>
                    </div>
                  </div>
                  ...
                  <div class="sub-comment-imgs" v-if="comment.images !== undefined && comment.images.length != 0">
                    <el-image :preview-src-list="[globalVar.STATIC_NAME + item]" style="width: 100px;"
                      :src="globalVar.STATIC_NAME + item" v-for="(item, index) in comment.images" :key="index" />
                  </div>
                </div>
                <div class="sub-comment-box" v-if="comment.subComments.length !== 0">
                  <div v-for="(sub_comment, subindex) in comment.subComments" class="comment-inner-box"
                    :style="{ height: comment.unfolded ? 'fit-content' : '0' }" :key="subindex">
                    <div class="sub-comment-header">
                      <el-avatar
                        :src="sub_comment.avatar == null ? 'https://cube.elemecdn.com/0/88/03b0d39583f48206768a7534e55bcpng.png' : globalVar.STATIC_NAME + comment.avatar"></el-avatar>
                      <div class="username-box">
                        {{ sub_comment.username === null ? 'id' + sub_comment.user_id : sub_comment.username }}
                        <i class="el-icon-caret-right" />
                        {{ sub_comment.reply_to_username === null || sub_comment.reply_to_username === '' ?
                          sub_comment.reply_to_comment_id : sub_comment.reply_to_username }}
                      </div>
                      <div class="time" style="flex-wrap: nowrap">
                        {{ sub_comment.createTime }}
                      </div>
                      <div class="time" style="flex-wrap: nowrap" v-if="sub_comment.rating != '-1'">
                        <el-rate v-model="sub_comment.rating" disabled show-score text-color="#ff9900"
                    ...
                    <div class="sub-comment-imgs"
                      v-if="sub_comment.images !== undefined && sub_comment.images.length != 0">
                      <el-image :preview-src-list="[globalVar.STATIC_NAME + item]" style="width: 100px;"
                        :src="globalVar.STATIC_NAME + item" v-for="(item, index) in sub_comment.images" :key="index" />
                    </div>
                  </div>
                  <div class="export-btn" @click="unfold(index)">
                    {{ comment.unfolded ? '收起' : '展开' }}{{ comment.subComments.length }}条评论</div>
                </div>
              </div>
            </div>
    ...
    <div>
      <el-dialog title="收货地址选择" :visible.sync="dialogVisible" width="40%">
        <el-table ref="singleTable" :data="tableData" highlight-current-row @current-change="handleCurrentChange1"
          style="width: 100%">
...
export default {
  ...
  methods: {
    getAverRating() {
      request.get(globalVar.HOST_NAME + "/comment/avgCommodityRating", {
        params: {
        ...
        console.log("平均评分")
        console.log(res)
        this.averRating = res.data.avgRating.toFixed(1);
      })
    },
    setCurrent(row) {
    ...
    addShip() {
      ...
      request({
        method: "POST", url: globalVar.HOST_NAME + "/shopcart/addtoshopcart", data: { "infoList": requestBody },
        params: params
      }).then(res => {
        console.log("加入购物车")
        this.$message({
    ...
    sureorder() {
      ...
      request({
        url: globalVar.HOST_NAME + "/user_order/create_one", method: "POST", data: {
          user_id: this.user_id,
